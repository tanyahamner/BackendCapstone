...
def create_all():
    with app.app_context():
        ...
        print("Querying for Hamner Livestock organization...")
        org_data = db.session.query(Organizations).filter(Organizations.name == "Hamner Livestock").first()
        if org_data == None:
            print("Hamner Livestock organization not found. Creating Hamner Livestock Organization in database...")
            name = 'Hamner Livestock'
        ...
        user_data = db.session.query(AppUsers).filter(AppUsers.email == 'ranch@hamnerlivestock.com').first()
        if user_data == None:
            print("Super Admin not found! Creating ranch@hamnerlivestock user...")
            first_name = 'Super'
            last_name = 'Admin'
            email = 'ranch@hamnerlivestock.com'
            newpw = ''
...
app.register_blueprint(routes.meats)
app.register_blueprint(routes.wools)
app.register_blueprint(routes.purchases)
